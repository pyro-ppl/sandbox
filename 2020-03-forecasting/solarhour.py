...
class StableLinearHMM(PyroModule):
    ...
    def get_dist(self, duration=None, obs_scale=None, obs_matrix_multiplier=None):
        obs_matrix = self.obs_matrix.repeat_interleave(hour, dim=-1)
...
def get_data(args=None):
    ...
    data = data.reshape(data.size(0) // hour, hour * data.size(-1))

    night_features = night_features[to_drop:to_drop + to_keep].double()
    night_features = night_features.reshape(night_features.size(0) // hour, hour * night_features.size(-1))

    covariates = covariates.reshape(covariates.size(0) // hour, hour * covariates.size(-1))
...
class Model(ForecastingModel):
    def __init__(self, trans_noise="gaussian", obs_noise="gaussian", state_dim=3, obs_dim=14, nightval=0.0):
        ...
        print("nightval", nightval, "obs_dim", obs_dim)
    ...
    def model(self, zero_data, covariates):
        ...
        night_covariates = night_covariates.reshape(night_covariates.size(0) * hour, num_stations)
        short_covariates = short_covariates.reshape(short_covariates.size(0) * hour, short_covariates.size(-1) // hour)
        #print("short_covariates2, night_covariates2", short_covariates.shape, night_covariates.shape)
        day_covariates = 1.0 - night_covariates
        T_minutes = short_covariates.size(0)

        which_hour = torch.arange(self.mean_granularity).repeat(1 + T_minutes // self.mean_granularity)[:T_minutes]
        ...
        obs_scale = self.obs_scale.index_select(0, which_hour)
        #print("obs_scale", obs_scale.shape)
        obs_scale = day_covariates * obs_scale + night_covariates * self.night_scale
        obs_scale = obs_scale.reshape(obs_scale.size(0) // hour, self.obs_dim)
        ...
        periodic = periodic.reshape(periodic.shape[:-2] + (periodic.size(-2) // hour, self.obs_dim))
        #if periodic.dim() == 3:
        #    periodic = periodic.squeeze(0)
        #print("periodic3", periodic.shape)
        obs_matrix_multiplier = day_covariates.reshape(day_covariates.size(0) // hour, self.obs_dim).unsqueeze(-2)
        #print("obs_matrix_multiplier", obs_matrix_multiplier.shape)
...
#        return pred.expm1().clamp(min=0.0), truth.expm1()
    ...
    metrics = backtest(data, covariates,
                       ...
                       train_window=None,
                       seed=args['seed'],
                       min_train_window=args['train_window'] * 24,
                       test_window=args['test_window'] * 24,
                       stride=args['stride'] * 24,
    ...
    for name in ["mae_fine", "crps_fine"]:
        values = np.stack([m[name].float() for m in metrics]).reshape((-1, day))
        results[name] = values

        metric_t = name[:-5] + '_{}'.format(1)
        ...
        mean = np.mean(values[:, index_val])
        std = np.std(values[:, index_val])
        results[metric_t + '_val'] = mean
        results[metric_t + '_val_std'] = std
        log("{} = {:0.4g} +- {:0.4g}".format(metric_t + '_val', mean, std))

        mean = np.mean(values[:, index_test])
        std = np.std(values[:, index_test])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--num-steps", default=8, type=int)
