...
class StableLinearHMM(PyroModule):
    ...
    def _get_obs_dist(self):
        if self.obs_noise == "stable":
            return dist.Stable(self.obs_stability, torch.zeros(self.obs_dim), scale=self.obs_noise_scale / root_two).to_event(1)
        else:
    ...
    def _get_trans_dist(self):
        if self.trans_noise == "stable":
            return dist.Stable(self.trans_stability, torch.zeros(self.state_dim), scale=self.trans_noise_scale / root_two).to_event(1)
        else:
