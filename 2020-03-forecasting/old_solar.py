...
from pyro.ops.tensor_utils import periodic_cumsum, periodic_features, periodic_repeat, convolve
...
class Model(ForecastingModel):
    ...
    def model(self, zero_data, covariates):
        ...
        #trans_scale = pyro.param("trans_scale", torch.tensor(1.0e-1), constraint=constraints.positive)
        ...
        with self.time_plate, poutine.reparam(config={"drift": LocScaleReparam()}):
            with poutine.reparam(config=self.trans_config):
                if self.trans_noise == "gaussian":
                    ...
                    drift = pyro.sample("drift", Stable(self.trans_stability, torch.zeros(zero_data.shape),
                                                        scale=trans_scale / root_two).to_event(1))
                elif self.trans_noise == "student":
                    drift = pyro.sample("drift", StudentT(self.trans_nu, torch.zeros(zero_data.shape), scale=trans_scale).to_event(1))

        #print("drift", drift.shape)
        timescale = pyro.param("timescale", 10.0 * torch.ones(num_stations), constraint=constraints.positive)
        time = torch.arange(float(duration))
        kernel = time.div(timescale.unsqueeze(-1)).neg().exp()
        kernel = kernel / kernel.sum()
        motion = convolve(drift.transpose(-1, -2), kernel, mode="same").transpose(-1, -2)

        if self.trans_noise != "none":
            prediction = periodic + station_bias + motion
        else:
            prediction = periodic + station_bias + motion
...
def main(**args):
    ...
    def svi_forecaster_options(t0, t1, t2):
        ...
        return {"num_steps": num_steps, "learning_rate": lr,
                ...
                "dct_gradients": True, "warm_start": False,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train-window", default=365 * 24 * 60, type=int)
